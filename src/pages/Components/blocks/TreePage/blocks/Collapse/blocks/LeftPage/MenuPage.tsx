...
const MenuPage = ({ setTabsItems, tabsItems, setActiveTabsKey }: any) => {
  const [selectedKeys, setSelectedKeys] = useState(['0-0']); // 选择所有的slot及以下树形结构的checkbox
  ...
  const allSelected = slotList.map((item) => {
    ...
    const childKeysList = item.child.map((t) => t.key);
    const sunKeysList = item.child
      .map((p) => p.children.map((o) => o.key))
      .flat(Infinity);

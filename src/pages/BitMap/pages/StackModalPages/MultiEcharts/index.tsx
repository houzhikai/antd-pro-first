...
const MultiEcharts = ({ echartsAxisNumber, echartsIndex, echartsWidth, echartsHeight }) => {
  const {
    echartsDataColor,
    singleModeData,
    scaleNumber,
  ...
  useEffect(() => {
    const newData: any = getNewData(singleModeData.data, scaleNumber, echartsAxisNumber, dots, xAxisList, yAxisList);
  ...
  return (
    <div
      ...
      style={{
        display: 'flex',
        width: echartsWidth * xAxisList + (scaleNumber > 1 ? 100 : 42),
        height: echartsHeight * yAxisList + (scaleNumber > 1 ? 100 : 42),
      }}
    >
        ...
        <div key={idx}>
          {item?.map((ref, index) => (
            <div
              ...
              ref={ref}
              style={{
                width: echartsWidth + (idx === 0 ? 42 : 0),
                height: echartsHeight + (index === 0 ? 42 : 0),
                margin: '0',
              }}
            ></div>
          ))}

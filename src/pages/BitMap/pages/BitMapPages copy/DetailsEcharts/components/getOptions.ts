...
const getGrid = (position) => {
  ...
  switch (position) {
    case 'hasXY':
      ...
      break;
    case 'hasX':
      grid = { width: '100%', height: 'calc(100% - 42px)', top: 0, left: 0 };
      break;
    case 'hasY':
      grid = { width: 'calc(100% - 42px)', height: '100%', top: 0, left: 0 };
      break;
    default:
...
export const getOptions = (
  ...
  const getMarkLine = (width) => {
    return {
      width: width,
      type: 'line',
      color: width > 2 ? (theme === 'dark' ? '#999' : '#aaa') : borderColor,
    };
  };

...
export const getAxisLabelInterval = (scaleNumber, xMax, yMax) => {
  const percentage = getRatioNumber(scaleNumber) * 100;
  let value: any = {};
  if (scaleNumber === 0.125) {
    value = {
      axisTick: { xInterval: 0, yInterval: 0 }, //  设置刻度线每个都展示
      axisLabel: { xInterval: 7, yInterval: 7 }, // 设置标签每隔5个展示（注意这里是从0开始计数的，所以应该设置为4）
    };
    ...
    value = {
      axisTick: {
        xInterval: xMax < 1024 || yMax < 1024 ? xMax / Math.sqrt(Math.max(scaleNumber, 1)) - 1 : 1024 / 32 - 1,
        yInterval: yMax < 1024 || yMax < 1024 ? yMax / Math.sqrt(Math.max(scaleNumber, 1)) - 1 : 1024 / 32 - 1,
      },
      axisLabel: {
        xInterval: xMax < 1024 || yMax < 1024 ? xMax / Math.sqrt(Math.max(scaleNumber, 1)) - 1 : 1024 / 32 - 1,
        yInterval: yMax < 1024 || yMax < 1024 ? yMax / Math.sqrt(Math.max(scaleNumber, 1)) - 1 : 1024 / 32 - 1,
